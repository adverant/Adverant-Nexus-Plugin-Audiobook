/**
 * Usage Tracking Middleware for ProseCreator-Audiobook
 *
...
const SERVICE_NAME = 'prosecreator-audiobook';
...
function detectOperation(req: Request): string {
  ...
  // ProseCreator-Audiobook-specific operations
  if (path.includes('/synthesize')) return 'audio_synthesis';
  if (path.includes('/narrate')) return 'narration';
  if (path.includes('/voice')) return 'voice_operation';
  if (path.includes('/tts')) return 'text_to_speech';
  if (path.includes('/chapter')) return 'chapter_processing';
  if (path.includes('/book')) return 'book_processing';
  if (path.includes('/segment')) return 'segment_processing';
  if (path.includes('/mix')) return 'audio_mixing';
  if (path.includes('/master')) return 'audio_mastering';
  if (path.includes('/effect')) return 'audio_effect';
  if (path.includes('/export')) return 'audio_export';
  if (path.includes('/convert')) return 'audio_conversion';
  if (path.includes('/job')) return 'job_operation';
  if (path.includes('/queue')) return 'queue_operation';
...
function isExemptPath(path: string): boolean {
  const exemptPaths = ['/health', '/healthz', '/ready', '/readiness', '/liveness', '/startup', '/metrics', '/ping', '/', '/version', '/audiobook/health'];
